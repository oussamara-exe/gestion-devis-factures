  ...
  const handleConvertir = async (id) => {
    if (window.confirm('Voulez-vous convertir ce devis en facture ?')) {
      try {
        ...
        const msg = error.response?.data?.message || error.message || 'Erreur lors de la conversion du devis';
        alert(msg);
      }
    }
  };
                ...
                <div className="flex gap-2">
                  ...
                  {d.statut === 'BROUILLON' && (
                    <button
                      ...
                      title="Validez d'abord le devis (bouton ✓) pour pouvoir le convertir en facture"
                    >
                      <FileText size={18} />
                    </button>
                  )}
